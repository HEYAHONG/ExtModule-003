...
{
    uint32_t isrflags   = READ_REG(uart1_get_handle()->Instance->SR);
    uint32_t cr1its     = READ_REG(uart1_get_handle()->Instance->CR1);
    if (((isrflags & USART_SR_RXNE) != RESET) && ((cr1its & USART_CR1_RXNEIE) != RESET))
    {
        if(uart1_rx_handler!=NULL)
        {
            uint8_t data=uart1_get_handle()->Instance->DR;
            uart1_rx_handler(&data,sizeof(data));
        }
    }
}
